...
export interface BoardCardProps {
  ...
  status: 'open' | 'closed';
  onRemove?: () => void;
  type?: 'college' | 'scholarship'; // ✅ Optional prop to determine API route
}
...
const BoardCard: React.FC<BoardCardProps> = ({
  ...
  status,
  onRemove,
  type = 'college' // ✅ default to 'college' for backward compatibility
}) => {
  ...
  const saveRequirements = useCallback(
    debounce(async (updatedRequirements: { item: string; checked: boolean }[]) => {
      try {
        ...
        console.log(`[${type}] Tracker ID:`, trackerId, "- Saving checklist:", updatedRequirements);

        if (!res.ok) {
          const err = await res.text();
          console.error(`❌ Failed to save ${type} checklist:`, err);
        } else {
          console.log(`✅ ${type} checklist saved!`);
        }
      } catch (err) {
