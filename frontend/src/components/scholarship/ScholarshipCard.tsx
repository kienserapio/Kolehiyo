      ...
      <div className="flex justify-between items-start gap-4 sm:gap-6 mb-5">
        <div className="flex-1 flex flex-col" style={{ gap: "14px" }}>
          ...
          <div className="text-[13px] sm:text-[14px] md:text-[16px]" style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
            <p><span className="font-bold">Application:</span> {application_start} - {application_end}</p>
            <p><span className="font-bold">Scholarship Type:</span> {scho_type}</p>
            <p><span className="font-bold">Benefits:</span> {benefits}</p>
          </div>
        </div>

        <div className="flex-shrink-0">
          <img src={logo_url} alt={`${name} logo`} className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 lg:w-32 lg:h-32 object-contain" />
        </div>
      </div>

      {/* Bottom Section */}
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 sm:gap-4">
        <div className={`${currentStatus.bg} ${currentStatus.text} font-bold flex items-center justify-center rounded-[25px] text-sm sm:text-base`} style={{ width: "140px", height: "35px", minWidth: "140px" }}>
